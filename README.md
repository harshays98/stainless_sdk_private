...
The Inferencing Python library provides convenient access to the Inferencing REST API from any Python 3.7+
...
The REST API documentation can be found on [docs.inferencing.com](https://docs.inferencing.com). The full API of this library can be found in [api.md](api.md).
...
Simply import `AsyncInferencing` instead of `Inferencing` and use `await` with each API call:
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `inferencing.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `inferencing.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `inferencing.APIError`.
...
try:
    client.inference.tasks.create(
        k_customer_id="K-Customer-ID",
    )
except inferencing.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except inferencing.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except inferencing.APIStatusError as e:
...
client.with_options(max_retries=5).inference.tasks.create(
    k_customer_id="K-Customer-ID",
)
```
...
client.with_options(timeout=5.0).inference.tasks.create(
    k_customer_id="K-Customer-ID",
)
```
...
You can enable logging by setting the environment variable `INFERENCING_LOG` to `debug`.

```shell
$ export INFERENCING_LOG=debug
```
...
task = response.parse()  # get the object that `inference.tasks.create()` would have returned
print(task.id)
```
