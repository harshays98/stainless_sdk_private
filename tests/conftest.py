...
from inferencing import Inferencing, AsyncInferencing, DefaultAioHttpClient
from inferencing._utils import is_dict
...
logging.getLogger("inferencing").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Inferencing]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Inferencing(base_url=base_url, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncInferencing]:
